...
{
    public partial class Form_Sabitler_Ders_Kayit : Form
    {
        public Form_Sabitler_Ders_Kayit()
        {
            InitializeComponent();
        }
        ...
        private void Form_Sabitler_Ders_Kayit_Load(object sender, EventArgs e)
        {
        ...
        private void button_Ders_Yeni_Kayit_Click(object sender, EventArgs e)
        {
        ...
        {
            OleDbDataAdapter yukle = new OleDbDataAdapter("SELECT * FROM Ders_Bilgileri", A_Genel.baglanti);
            DataSet ds = new DataSet();
            yukle.Fill(ds, "Ders_Bilgileri");
            dataGridView_Ders_Kayit.DataSource = ds.Tables["Ders_Bilgileri"];
            dataGridView_Ders_Kayit.Columns["Ders_id"].Visible = false;
        }

        private void button_Ders_Sil_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                string sorgu = "DELETE From Ders_Bilgileri WHERE Ders_id=" + satir + "";
        ...
        private void button_Ders_Kaydet_Click(object sender, EventArgs e)
        {
            if (A_Genel.anahtar)
            {
                try
                {
                    string sorgu = "INSERT INTO Ders_Bilgileri(Ders_Adi,Aciklama)values(@1,@2)";
                    komut = new OleDbCommand(sorgu, A_Genel.baglanti);
            ...
            else
            {
                try
                {
                    ...
                    string sorgu = "UPDATE Ders_Bilgileri SET Ders_Adi=@1,Aciklama=@2 WHERE Ders_id=" + guncel_satir + "";
